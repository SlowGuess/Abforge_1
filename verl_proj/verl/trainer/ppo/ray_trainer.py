...
def compute_reward_metrics(data: List[Dict], prefix: str = ""):
    ...
    numeric_score_detail_df = score_detail_df.select_dtypes(include=[np.number])
    metrics = {}

    first_tag = "reward_detail" if len(prefix) == 0 else f"{prefix}/reward_detail"

    if numeric_score_detail_df.shape[1] == 0:
        return metrics
    ...
    for name, func in zip(
        ...
        stat_data = func(numeric_score_detail_df, axis=0).to_dict()
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.config.algorithm.adv_estimator == AdvantageEstimator.REMAX:
                        with _timer("gen_max", timing_raw):
                            ...
                            # 修改后：
                            print(f"🔍 DEBUG: reward_baseline_tensor type: {type(reward_baseline_tensor)}")

                            # 如果是元组，提取第一个元素（reward_tensor）
                            if isinstance(reward_baseline_tensor, tuple):
                                print("🔍 从元组中提取 reward_tensor")
                                reward_baseline_tensor = reward_baseline_tensor[0]  # 提取 reward tensor

                            print(f"🔍 DEBUG: 处理后 reward_baseline_tensor shape: {reward_baseline_tensor.shape}")
