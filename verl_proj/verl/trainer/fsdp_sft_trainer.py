...
class FSDPSFTTrainer(object):
    ...
    def fit(self):
        ...
        # save_freq behavior:
        #   <= 0 : save only once at the very end of training
        #   >  0 : save every N global steps, plus once at the end
        save_freq = int(self.config.trainer.get('save_freq', -1))

        for epoch in range(self.config.trainer.total_epochs):
            is_last_epoch = (epoch == self.config.trainer.total_epochs - 1)
            ...
            for data in tqdm(self.train_dataloader,
                ...
                # periodic mid-training save (only when save_freq > 0)
            ...
            # epoch-end checkpoint: save only on the very last epoch when
            # save_freq <= 0, or on every epoch when save_freq > 0.
            if save_freq > 0 or is_last_epoch:
                self.save_checkpoint(step=global_step)
