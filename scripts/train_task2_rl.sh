...
MODEL_PATH=${MODEL_PATH:-$ABFORGE_ROOT/outputs/checkpoints/task2_sft}
TRAIN_PATH=${TRAIN_PATH:-$ABFORGE_ROOT/data/abforge_task2_rl/train.parquet}
VAL_PATH=${VAL_PATH:-$ABFORGE_ROOT/data/abforge_task2_rl/val.parquet}
REWARD_URL=${REWARD_URL:-http://127.0.0.1:6011/get_reward_task2}
...
python3 -m verl.trainer.main_ppo \
  ...
  trainer.project_name="${PROJECT_NAME:-abforge_task2}" \
  trainer.experiment_name="${EXPERIMENT_NAME:-qwen3_8b_grpo_task2}" \
  ...
  trainer.default_local_dir="${SAVE_DIR:-$ABFORGE_ROOT/outputs/checkpoints/task2_rl}" \
  "$@"
