...
TRAIN_PATH=${TRAIN_PATH:-$ABFORGE_ROOT/data/abforge_task1_sft/train.parquet}
VAL_PATH=${VAL_PATH:-$ABFORGE_ROOT/data/abforge_task1_sft/val.parquet}
SAVE_DIR=${SAVE_DIR:-$ABFORGE_ROOT/outputs/checkpoints/task1_sft}
...
torchrun --standalone --nnodes=1 --nproc_per_node="$NPROC_PER_NODE" \
  ...
  trainer.project_name="${PROJECT_NAME:-abforge_task1_sft}" \
  trainer.experiment_name="${EXPERIMENT_NAME:-qwen3_8b_sft_task1}" \
